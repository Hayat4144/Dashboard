                        ...
                        <section className='transaction_data rounded-md shadow-2xl my-10 mx-2 md:mx-5 lg:mx-10 overflow-x-auto dark:bg-gray-800'>
                            <table className='table-auto w-full'>
                                ...
                                <tbody className='mb-2 text-left'>
                                    {products.map((product, index) => <Fragment>
                                        <tr key={index} className="border-b border-gray-300 dark:border-gray-500 cursor-pointer  hover:bg-gray-200 dark:hover:bg-gray-700 dark:text-gray-200">
                                            <td className='px-4 py-2'>
                                                <figure>
                                                    {
                                                        product.assets.images.map((image, index) => (
                                                            <img src={image.url} alt="product-image" key={index} className="w-20 h-10 rounded-md hover:scale-150" />
                                                        ))
                                                    }
                                                </figure>
                                            </td>
                                            ...
                                            <td className="px-2 py-2"><Link to={`/v3/seller/edit/product/${product.id}`}><EditOutlinedIcon /></Link></td>
                                            <td className="px-2 py-2" onClick={() => handleDelete(product._id)}><DeleteOutlineOutlinedIcon /></td>
                                        </tr>
                                    </Fragment>)}
                                </tbody>
                            </table>
